...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
    ...
    if (!node->bounds.IntersectP(ray, ray.direction_inv, dirIsNeg))//如果这里没有碰上当前节点的bounding_box的话，直接退出
        return isect;
    
    if (node->is_leaf()) {//这里如果是叶节点的话
    ...
    if (isect1.distance < isect2.distance)//谁先被光线碰到返回谁
        return isect1;
    else
        return isect2;
