...
static bool insideTriangle(int x, int y, const Triangle& t){
    ...
    return ((r[0][2] > 0) && (r[1][2] > 0) && (r[2][2] > 0)) || ((r[0][2] < 0) && (r[1][2] < 0) && (r[2][2] < 0));
}
...
{
    auto v = t.toVector4(); //std::array<Eigen::Vector4f, 3> res;
    struct bounding_box {
        float x_max = -9999.0f,
              x_min = 9999.0f,
              y_max = -9999.0f,
              y_min = 9999.0f;//准备建立bounding box
    }box;
    for (auto it = v.begin(); it != v.end(); ++it) {
        if ((*it)[0] > box.x_max) box.x_max = (*it)[0];
        if ((*it)[0] < box.x_min) box.x_min = (*it)[0];
        if ((*it)[1] > box.y_max) box.y_max = (*it)[1];
        if ((*it)[1] < box.y_min) box.y_min = (*it)[1];
    }
    for (int x = box.x_min - 1; x < box.x_max; ++x) {
